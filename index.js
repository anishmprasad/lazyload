	...
	componentDidMount() {
		if (typeof process !== 'undefined' && process.env.NODE_ENV !== 'production') {
			if (React.Children.count(this.props.children) > 1) {
				console.warn('[lazyload] Only one child is allowed to be passed to `LazyLoad`.');
			}

			if (this.props.wheel) {
				// eslint-disable-line
				console.warn(
					'[lazyload] Props `wheel` is not supported anymore, try set `overflow` for lazy loading in overflow containers.'
				);
			}
			...
			if (
				...
				ReactDom.findDOMNode(this).offsetHeight === 0
			) {
				console.warn('[lazyload] Please add `height` props to <LazyLoad> for better performance.');
			}
		}
		...
		if (this.props.debounce !== undefined && delayType === 'throttle') {
			console.warn(
				'[lazyload] Previous delay function is `throttle`, now switching to `debounce`, try setting them unanimously'
			);
			needResetFinalLazyLoadHandler = true;
		} else if (delayType === 'debounce' && this.props.debounce === undefined) {
			console.warn(
				'[lazyload] Previous delay function is `debounce`, now switching to `throttle`, try setting them unanimously'
			);
			needResetFinalLazyLoadHandler = true;
		}
